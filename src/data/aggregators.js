...
export function aggregateBrudd(rapporter) {
  const byKey = {};
...
export function aggregateBruddByMyndighet(rapporter) {
  const groups = {};
  for (const r of rapporter) {
...
export function groupByMyndighet(rows) {
  const res = {};
  for (const r of rows) {
    const m = r?.tilsynsmyndighet || "Ukjent";
    if (!res[m]) res[m] = [];
