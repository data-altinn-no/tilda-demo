import { randInt, pad } from './randomHelpers';
...
export function randomDateISOYearAround(): string {
  const now = new Date();
...
export function randomDateInRange(fromDate?: string | null, toDate?: string | null): string {
...
export function randomFutureDateISO(monthsAheadMin: number = 1, monthsAheadMax: number = 9): string {
  const now = new Date();
