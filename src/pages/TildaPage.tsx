...
export function TildaPage() {
  ...
  const [koord, setKoord] = useState<any[]>([]);
  ...
  const [relatedCompanies, setRelatedCompanies] = useState<any>(null);
  const [presetCompanyName, setPresetCompanyName] = useState<string | null>(null);
  ...
  const toDate = today;
  const [searchFromDate, setSearchFromDate] = useState<string | null>(null);
  const [searchToDate, setSearchToDate] = useState<string | null>(null);
  ...
  const getStatusColor = (): string => {
    if (bruddCount === 0) return "text-green-500";
    if (rap.length === 0) return "text-gray-500";
  ...
  const allAuthorities = useMemo(() => {
    ...
    rap.forEach((r: any) => r.tilsynsmyndighet && authorities.add(r.tilsynsmyndighet));
    koord.forEach((k: any) => k.tilsynsmyndighet && authorities.add(k.tilsynsmyndighet));
          ...
          <div className="glass-card rounded-xl p-2 -mt-4">
            <div className="flex items-center gap-1.5 flex-wrap">
              {allAuthorities.map((authority) => {
                ...
                const authorityBrudd = perMynd[authority]?.reduce((sum: number, item: any) => sum + item.brudd, 0) || 0;
                return (
                  <button
                    key={authority}
