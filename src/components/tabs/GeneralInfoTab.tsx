...
interface AuthorityTooltipProps {
  authority: string;
}
...
interface GeneralInfoTabProps {
  ...
  rap: any[];
  koord: any[];
  perMynd: Record<string, any[]>;
  onAuthorityClick: (authority: string) => void;
 ...
 */
function generateDynamicPeriods(fromDate: string | null, toDate: string | null, numPeriods: number = 10): any[] {
  const periods: any[] = [];
 ...
 */
function aggregateTilsynByYear(rap: any[], fromDate: string | null) {
 ...
 */
function isGazelleOrganisation(financialData: any, orgDetails: any): boolean {
 ...
 */
function InfoTooltip({ text, position = 'top' }: InfoTooltipProps) {
 ...
 */
function AuthorityTooltip({ authority }: AuthorityTooltipProps) {
  const info = AUTHORITY_INFO[authority as keyof typeof AUTHORITY_INFO];
  if (!info) return null;
    ...
    rap.forEach(r => {
      ...
      if (yearData[year]) {
        ...
        if (r.funn && r.funn.length > 0) {
          r.funn.forEach(f => {
            if ((f as any).reaksjonstype && (f as any).reaksjonstype !== 'Ingen') {
  ...
  return (
    <Card>
      ...
      <CardContent className="grid gap-4">
        ...
        >
          ...
          <div className="grid md:grid-cols-4 gap-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg hover:bg-gray-100 transition-colors">
              ...
              <div className={`text-2xl font-bold ${getStatusColor(bruddCount)}`}>{bruddCount}</div>
        ...
        {/* Related Companies */}
        {relatedCompanies && (relatedCompanies as any).companies && (relatedCompanies as any).companies.length > 0 && (
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="text-sm text-gray-600 mb-2 flex items-center gap-2">
              <Building2 className="w-4 h-4" />
              {(relatedCompanies as any).type === 'parent' ? 'Datterselskaper' : 'Søsterselskaper'} ({(relatedCompanies as any).companies.length})
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {(relatedCompanies as any).companies.map((company: any) => (
                <div key={company.organisasjonsnummer} className="flex items-center justify-between bg-white p-3 rounded border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors">
                  <div>
