...
import { Zap, Info, LineChart as LineChartIcon } from 'lucide-react';
 ...
 */
export function ExperimentTab({ rap, selectedAuthority, onClearSelection }) {
  const mapRef = useRef(null);
  const mapInstanceRef = useRef(null);

  // Extract cities from addresses and count violations
  const cityViolations = {};
  ...
  filteredRap.forEach(r => {
    if (r.tilsynsadresse && isBrudd(r)) {
      // Extract city from address (last part after comma and space)
      const addressParts = r.tilsynsadresse.split(', ');
      if (addressParts.length >= 2) {
        const cityPart = addressParts[addressParts.length - 1];
        // Remove zipcode (first 4-5 digits) to get city name
    ...
    <Card className="rounded-2xl">
      ...
      <CardContent>
        <div className="grid gap-6">
          {/* Norway Map with Violation Dots */}
          <NorwayMap 
